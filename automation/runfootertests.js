...
for await (const line of rl) {
    ...
    const results = await Promise.all([FooterCheck(website)]);

    let json = {"website": website};
    results.forEach((result, index) => {
        json["footer"] = result === "fail" ? "False" : result === "pass" ? "True" : "Disabled";
        json["isPhishing"] = false;
    });
    console.log(JSON.stringify(json));
    allResults.push(json);
}

console.error(JSON.stringify(allResults));
